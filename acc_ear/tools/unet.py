...
UNet 主体网络结构
    下降 -> 底 -> 上升，共5层，下降/上升4次
    下降：每层进行4*4卷积，步长2、ReLU，共两次，然后向下2*2最大池化进行缩减；
    上升：每层进行4*4卷积，步长2、ReLU，共两次，然后向上2*2反卷积进行放大；
...
参考博客
    UNet：https://zhuanlan.zhihu.com/p/1913526482736751771
'''

# 简化每层的卷积操作
class DownUpConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=2, padding=1):
        # 将kernel_size改为4，stride改为2，符合论文要求
...
class UNet(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.stage_down1=DownUpConv(3, 64)
        self.stage_down2=DownUpConv(64, 128)
        self.stage_down3=DownUpConv(128, 256)
        self.stage_down4=DownUpConv(256, 512)
        self.stage_down5=DownUpConv(512, 1024)
        ...
        self.stage_up4=DownUpConv(1024, 512)
        self.stage_up3=DownUpConv(512, 256)
        self.stage_up2=DownUpConv(256, 128)
        self.stage_up1=DownUpConv(128, 64)
        self.stage_out=DownUpConv(64, num_classes)
